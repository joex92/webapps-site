...
export function getWordPoints(word,x=10-width/2,y=height/2-10,size=100,space=size/10,font=digital){
...
export function planetGrav(mass=EARTH_M,atRadius=EARTH_R){
  return G*(mass/Math.pow(atRadius,2));
}

export function sliderVolume(cnv = p5.instance._renderer,x,y,gray = 50,alpha = 100,w = innerHeight/60,h = innerHeight/10){
...
export function propsAsString(obj={},sep=" ") {
...
export function toFixed(obj, fixed) {
if (objType(obj) == "Number") {
...
export function ms2ftime(ms, ly = 0) {
...
export function rgb2rgba(src, a = 1) {
...
export function freq2wavelength(freq=440,c=C_VACUUM){
    return (c/freq)*NM; // Wavelength in nanometers
}

export function wavelength2freq(wavelength=freq2wavelength(440*TO40OCT),c=C_VACUUM){
    wl = wavelength/NM;
    return (c/wl);
}

// takes wavelength in nm and returns an rgba value
export function wavelength2color(wavelength) {
    var R,
        G,
        B,
        alpha,
...
export function freq2color(freq=440){
    return wavelength2color(freq2wavelength(freq*TO40OCT));
}

export function midi2color (midi=69) {
    let f,m,d;
    if (midi >= 66 && midi <= 77){
        d = 0;
    } else if (midi < 66) {
